import{$ as o,H as S}from"./header-9Fuft6xP.js";import{F as $}from"./footer-pXvuOvyE.js";import{a as v}from"./firebase-IcnoC1y6.js";import{g as w,a as x,c as k,d as g,b as L,u as q,e as E,s as C}from"./index.esm2017-MrW1TCTu.js";import{b as A,c as j}from"./swiper-cPm6rBfW.js";import{P as H}from"./preloader-a9ovPH_o.js";import{o as O,g as P}from"./index-e24386e7-IBhCI8Y3.js";class K{dropDown;dropMain;quatityAdults;quatityKids;addBtn;btn;children;tegContainer;error;currentAbdul;currentKids;customEvent;constructor(t){this.dropDown=o(t).find(".dropdown"),this.dropMain=this.dropDown.find(".dropdown__main"),this.quatityAdults=this.dropDown.find("#adults-quantity"),this.quatityKids=this.dropDown.find("#kids-quantity"),this.addBtn=this.dropDown.find(".dropdown__button-add"),this.error=this.dropDown.find(".dropdown__error"),this.btn=this.dropDown.find(".dropdown__btn"),this.children=this.dropDown.find(".dropdown__children"),this.tegContainer=o(".tour__quantity-tegs"),this.customEvent=new CustomEvent("myCustomEvent"),this.currentAbdul=1,this.currentKids=0,this.init()}init(){this.open(),this.counter(this.dropDown.find(".dropdown__count-one")),this.counter(this.dropDown.find(".dropdown__count-two"),!0),this.removeTag(),this.removeAgeSelect(1),this.addPeople()}open(){this.btn.on("click",()=>{this.dropDown.toggleClass("dropdown_active")});const t=this;o(document).on("click",function(r){t.dropDown.has(r.target).length==0&&t.dropDown.removeClass("dropdown_active")})}counter(t,r=!1){const s=t.find(".dropdown__count-add"),e=t.find(".dropdown__count-remove"),i=t.find(".dropdown__count-current"),n=this;function l(){let c=+i.text();if(c<9){const a=c+1;i.html(String(a)),r?n.timeout(a,1):n.currentAbdul=a}}function d(){let c=+i.text();if(c>1||r&&c>0){const a=c-1;i.html(String(a)),r?(n.timeout(a,2),o(".dropdown__error").removeClass("dropdown__error_active-kids")):n.currentAbdul=a}}s.on("click",l),e.on("click",d)}addAgeSelect(t,r=!1){const e=this.dropDown.find(".children__drop").length;if(console.log(e),r){const i=this.kids();console.log(i),this.children.html("");let n=0;for(const l in i)for(let d=0;d<+i[l];d++){this.children.append(`
        ...
        `);for(const s in t)this.tegContainer.append(`
          ...
          `);document.dispatchEvent(this.customEvent)}})}getAbduls(){return this.currentAbdul}getKids(){return this.currentKids}}class I{totalPrice;dropDown;constructor(){this.totalPrice=o(".tour__total-price"),this.dropDown=new K(".tour__quantity-drop"),this.priceCalculator()}priceCalculator(){document.addEventListener("myCustomEvent",()=>{const t=o(".tour__price-sum").data("price"),r=this.dropDown.getKids()+this.dropDown.getAbduls(),s=String(+t*r);let e="";if(s.length>3){let i=0;for(let n=s.length-1;n>=0;n--)i++,e+=s[n],i===3&&(e+=" ");e=e.split("").reverse().join("")}else e=s;this.totalPrice.html(e),console.log(s)})}}class F{app;currentUrl;id;constructor(){this.app=v,this.currentUrl=new URL(window.location.href),this.id=this.currentUrl.searchParams.get("id")||null,this.init()}async init(){this.addQuery(),this.getData()}map(t,r){const i=o(".top-img__hotel-info").attr("href")?.replace(/query=/,`query=${t}, ${r}`);i&&o(".top-img__hotel-info").attr("href",i)}addQuery(){this.currentUrl.search="",this.currentUrl.searchParams.append("id","2xvc7W065aSKXG4o5mw7"),history.replaceState({},"",this.currentUrl)}async getData(){const t=w(this.app),r=await x(k(t,"hotels"));let s=0;const e=r.docs.length-1;r.forEach(i=>{const n=+(Math.random()*e).toFixed();i.id===this.id?this.changeInfo(i.data()):i.id!==this.id&&s<4&&(s++,this.bottomSlide(r.docs[n].data(),r.docs[n].id))}),new A,new H}bottomSlide(t,r){const s=o("#bottom-slider"),e=t,i=e.name,n=this.dateFormat(e.date).replace(" г.",""),l=e.country,d=e.beach,c=e.star,a=e.img[0],p=+e.price[0],_=+e.price[1],u=_?+((p-+_)/p*100).toFixed():!1,b=()=>{let m="";for(let f=0;f<c;f++)m+='<svg><use xlink:href="#start"></use></svg>';return m},y=`
        ...
        `)}),new j}}class M{objData;orderBtn;user;currentUrl;constructor(){this.currentUrl=new URL(window.location.href),this.objData={},this.objData.id=this.currentUrl.searchParams.get("id")||null,this.orderBtn=o(".tour__btn"),this.user=!1,this.init()}init(){this.getSing(),this.setOrder(),this.dateOrder()}getSing(){O(P(),t=>{t&&(this.user=t,this.objData.email=t.email?t.email:null)})}dateOrder(){const t=new Date,r=t.getTime();this.objData.idOrder=+String(r).slice(3),this.objData.date=this.dateFormat(t)}setOrder(){this.orderBtn.on("click",async()=>{if(typeof this.user!="boolean"){const t=this.user.uid,r=w(v);this.objData.price=+o(".tour__total-price").html().replace(" ",""),this.objData.payStatus=!1;const s=g(r,"users",t);(await L(s)).exists()?await q(s,{orders:E(this.objData)}):await C(g(r,"users",t),{orders:[this.objData]}),confirm("Тур добавлен и ожидает оплаты, перейти в личный кабинет?")&&(window.location.href="Account.html")}else confirm("Чтобы забронировать тур необходимо авторизоваться, перейти на страницу авторизации?")&&(window.location.href="authorization.html")})}dateFormat(t){const r=["января","февраля","марта","апреля","мая","июня","июля","августа","сентября","октября","ноября","декабря"],s=t.getDate(),e=r[t.getMonth()],i=t.getFullYear(),n=t.getHours(),l=t.getMinutes();return`${s} ${e} ${i} ${n}:${l}`}}document.addEventListener("DOMContentLoaded",()=>{class h{infoBlock;header;footer;pullData;order;constructor(){this.infoBlock=new I,this.header=new S,this.footer=new $,this.pullData=new F,this.order=new M,this.init()}init(){}}new h});
